...
class Cmat:
    ...
    def solve(self, right, left):
        FNr = self.Nmat.solve(right, self.Fmat)
        FNl = FNr if (left is right) else self.Nmat.solve(left, self.Fmat)
        return self.Nmat.solve(right, left) - np.dot(FNl.T, scipy.linalg.cho_solve(self.Sigma, FNr))
...
class OptimalStatistic:
    def __init__(self, psrs, pta, params, residuals=None):
        ...
        for ii in range(Nfreqs):
            ...
            start_idx = 2 * ii
            end_idx = 2 * (ii + 1)
            gamma_tmp[start_idx:end_idx] = self.phimat[start_idx:end_idx]
            # a . np.diag(g) . b = a . (g * b) = (a * g) . b
    ...
    def set_residuals(self, residuals):
        """reset residuals and recompute rhos and sigmas and necessary matrices

        Args:
            residuals (np.ndarray): list of new residuals. each list entry is an array of residuals for each pulsar
            ordered according to self.psrs.
        """
    ...
    def setup_mcos(self, orfs):
        """set up information for multicomponent optimal statistic. Meant to be a helper method.

        Args:
            orfs (list): list of callable functions to calculate overlap reduction function.
        """
    ...
    def mcos(self, orfs=None):
        """calculate multicomponent optimal statistic

        Args:
            orfs (list, optional): list of callable overlap reduction functions. Defaults to None.

        Returns:
            ahat: maximum likelihood amplitude
            sigma: uncertainty on ahat
        """
        if orfs is None:
            orfs = [utils.monopole_orf, utils.dipole_orf, utils.hd_orf]
        elif not isinstance(orfs, list) and isinstance(orfs, Callable):
            orfs = [orfs]
        else:
            raise ValueError("orfs must be a callable or a list of callables")
    ...
    def os_frequencies(self, orf=utils.hd_orf, sel=(lambda p: slice(None))):
        """TODO: allow for selections in this function. Currently they are not used.

        Args:
            orf (Callable, optional): Callable function for ORF. Defaults to utils.hd_orf.
            sel (selection, optional): function for selecting which pairs to include.
                Currently not implemented. Defaults to (lambda p: slice(None)).

        Returns:
            os_frequencies: optimal statistic calculated on frequency-by-frequency basis
        """
        self._set_orf(orf)

        mask = sel(self.pairs)

        return (np.sum(self.orfs * self.rhos_freqs * self.sigmas_freqs**-2, axis=1)
                / np.sum(self.orfs**2 * self.sigmas_freqs**-2, axis=1))

    def os_frequencies_sigmas(self, orf=utils.hd_orf, sel=(lambda p: slice(None))):
        """uncertainty on os_frequencies

        Args:
            orf (Callable, optional): callable funcion for overlap reduction function. Defaults to utils.hd_orf.
            sel (Selection, optional): enterprise selection object. not implemented currently.
                Defaults to (lambda p: slice(None)).

        Returns:
            os_frequencies_error: error on os_frequencies
        """
        self._set_orf(orf)
    ...
    def _tracedot(self, orf, *args):
        ...
        for i, (j, k) in zip(args[::2], args[1::2]):
            ret = np.dot(ret, self.FCFs[i] * self.spectral_model)
    ...
    def binned_os_with_correlations(self, nbins=11, orf=utils.hd_orf, binedges=None):
        ...
        for ii in range(1, js.size):
            ...
            for jj in range(1, js.size):
                ...
                bot = (ubin_ii.T @ np.linalg.inv(cov_tmp_ii) @ ubin_ii) * \
                    (ubin_jj.T @ np.linalg.inv(cov_tmp_jj) @ ubin_jj)
    ...
    def chi2_binned_covariances(self, nbins=11, binedges=None, orf=utils.hd_orf):
        ...
        return resids.T @ np.linalg.inv(Bjk) @ resids
    ...
    def chi2_binned_no_covariances(self, nbins=11, orf=utils.hd_orf):
        ...
        resids = (os_vals - get_HD_curve(angles) * 10**(2 * self.params['gw_log10_A']))
        return resids.T @ np.diag(errors**-2) @ resids

    def chi2_covariances(self):
        resids = self.rhos - 10**(2 * self.params['gw_log10_A']) * self.orfs
        return (resids.T @ np.linalg.inv(self.rho_covariance) @ resids)

    def chi2_no_covariances(self):
        resids = self.rhos - 10**(2 * self.params['gw_log10_A']) * self.orfs
        return (resids.T @ np.diag(np.diag(self.rho_covariance)**-1) @ resids)
